...
### A. Domain Layer (`{ProjectName}.Domain`)
- **Purpose**: Core business model and rules.
- **Rules**:
  - Must have **zero dependencies** on other projects, except for `{ProjectName}.Shared`.
...
### B. Application Layer (`{ProjectName}.Application`)
- **Purpose**: Application-specific business logic and orchestration.
- **Rules**:
  - References only `{ProjectName}.Domain` and `{ProjectName}.Shared`.
...
### C. Infrastructure Layer (`{ProjectName}.Infrastructure`)
- **Purpose**: External concerns and data access.
- **Rules**:
  - References `{ProjectName}.Application` and `{ProjectName}.Shared`.
  - Contains: DbContext implementations (`{ProjectName}DbContext`), EF configurations, migrations, repository implementations (`UserRepository`), Identity authentication services (`IdentityService`), and external mail delivery (`SmtpEmailService`).
  - Implements the interfaces defined in the Application layer.

### D. Presentation Layer (`{ProjectName}.Presentation`)
- **Purpose**: Entry point (HTTP API Web API).
- **Rules**:
  - References `{ProjectName}.Infrastructure`, `{ProjectName}.Application`, and `{ProjectName}.Shared`.
...
### E. Shared Layer (`{ProjectName}.Shared`)
...
- **Infrastructure Abstractions**: Always use repository interfaces (`IPostRepository`) inside Use Cases. Never inject `{ProjectName}DbContext` directly into Application layer handlers.
