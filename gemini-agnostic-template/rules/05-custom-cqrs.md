...
- **Scope**: {ProjectName}.Application & {ProjectName}.Presentation
...
## 1. Marker Interfaces (`{ProjectName}.Application/Common/CQRS/`)
...
### The Command:
```csharp
namespace {ProjectName}.Application.UseCases.Posts.Commands;
...
namespace {ProjectName}.Application.UseCases.Posts.CommandHandlers;
...
Every handler must be registered explicitly as `Scoped` inside `{ProjectName}.Application/Extensions/DependencyInjection.cs`:
```csharp
