...
## Overview
This codebase avoids raising exceptions for expected business errors (e.g., entity validation failed, record not found, authorization denied). Instead, it uses a functional **Result** pattern defined in `{ProjectName}.Shared` to represent outcomes explicitly. Exceptions are reserved strictly for exceptional runtime failures (database connection drop, system crash).

---

## 1. Shared Error and Result Primitives (`{ProjectName}.Shared`)

All projects in the solution (`{ProjectName}.Domain`, `{ProjectName}.Application`, `{ProjectName}.Infrastructure`, `{ProjectName}.Presentation`) must reference the `{ProjectName}.Shared` project:
