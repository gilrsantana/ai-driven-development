...
### 1. Identify Folders
Organize code in `src/{ProjectName}.Application/UseCases/{AggregateName}/`:
...
- **Example Command** (with return value):
  ```csharp
  using {ProjectName}.Application.Common.CQRS;
  using {ProjectName}.Shared;

  namespace {ProjectName}.Application.UseCases.Products.Commands;
...
- **Example Query**:
  ...
  namespace {ProjectName}.Application.UseCases.Products.Queries;
...
- **Example Handler**:
  ...
  using {ProjectName}.Application.UseCases.Products.Commands;
  using {ProjectName}.Shared;

  namespace {ProjectName}.Application.UseCases.Products.CommandHandlers;
...
- Open `src/{ProjectName}.Application/Extensions/DependencyInjection.cs`.
