...
Based on the selected provider, install the appropriate NuGet package inside the `src/{ProjectName}.Infrastructure` project:

- **PostgreSQL**:
  `dotnet add src/{ProjectName}.Infrastructure package Npgsql.EntityFrameworkCore.PostgreSQL`
- **SQL Server**:
  `dotnet add src/{ProjectName}.Infrastructure package Microsoft.EntityFrameworkCore.SqlServer`
- **MySQL**:
  `dotnet add src/{ProjectName}.Infrastructure package MySql.EntityFrameworkCore`
- **SQLite**:
  `dotnet add src/{ProjectName}.Infrastructure package Microsoft.EntityFrameworkCore.Sqlite`
- **Oracle**:
  `dotnet add src/{ProjectName}.Infrastructure package Oracle.EntityFrameworkCore`

### 2. Create the DatabaseOptions POCO
Create `src/{ProjectName}.Infrastructure/Configurations/DatabaseOptions.cs` to hold database parameters:
```csharp
...
Update `src/{ProjectName}.Infrastructure/Extensions/DependencyInjection.cs`:
