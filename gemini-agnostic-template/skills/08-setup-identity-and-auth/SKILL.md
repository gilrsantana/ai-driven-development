...
- **AccountRoleConfiguration**:
  ...
  ```
Apply the configurations using `builder.ApplyConfigurationsFromAssembly` inside the `{ProjectName}DbContext` class.
...
Create an interface `IIdentityService` in the Application layer, and implement it in `{ProjectName}.Infrastructure/Identity/IdentityService.cs`. Use `UserManager<Account>` and `RoleManager<Role>` to manage credentials:
