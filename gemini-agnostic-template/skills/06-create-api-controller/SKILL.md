...
- Create a file inside `src/{ProjectName}.Presentation/Controllers/` named `{Name}sController.cs` (e.g., `ProductsController.cs`).
- Namespace must be `{ProjectName}.Presentation.Controllers`.
...
using {ProjectName}.Application.UseCases.Products.Queries;
